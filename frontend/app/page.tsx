  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const markdownToHtml = (markdown: string) => {
        ...
        let nakshatraSummaryOpen = false;
        ...
        const closeNakshatraSummary = () => {
          if (!nakshatraSummaryOpen) {
            return "";
          }

          nakshatraSummaryOpen = false;
          return "</div>";
        };

        const closeSpecialBlocks = () =>
          `${closeNakshatraSummary()}${closeDashaEmphasisBox()}${closeTransitAnalysisSection()}${closeRoadmapYear()}${closeFinalObservation()}`;
        ...
        const html = lines
          .map((line) => {
            ...
            const nakshatraMatch = nakshatraSummaryMatch(trimmed);

            if (nakshatraMatch) {
              const content = `<p><strong>${escapeHtml(
                nakshatraMatch[1]
              )}:</strong> <strong>${escapeHtml(nakshatraMatch[2])}</strong></p>`;

              if (nakshatraSummaryOpen) {
                return content;
              }

              nakshatraSummaryOpen = true;
              return `${closeDashaEmphasisBox()}<div class="nakshatra-summary-card">${content}`;
            }

            return `${closeNakshatraSummary()}${closeDashaEmphasisBox()}<p>${escapeHtml(trimmed)}</p>`;
          })
          .join("");
